...
if __name__=='__main__':
    ...
    for seq_len in seq_lens:
        ...
        model = False
        if (single_branch):
            model = KerasRegressor(build_fn=bidir_lstm_cnn.build_model, validation_split = 0.20)
        else:
        ...
        for index, row in results.iterrows():
            top_model = False
            if (single_branch):
                top_model = bidir_lstm_cnn.build_model(row['param_layers'], row['param_cnn_layers'], row['param_kernel_size'])
            else:
