...
import lstm
import cnn_batchnorm_bidir_lstm
...
if __name__=='__main__':
    ...
    description = 'bidir_lstm_single_after_cnn'
    ...
    for seq_len in seq_lens:
        ...
        model = False
        if (single_branch):
            model = KerasRegressor(build_fn=cnn_batchnorm_bidir_lstm.build_model, validation_split = 0.20)
        else:
        ...
        for index, row in results.iterrows():
            top_model = False
            if (single_branch):
                top_model = cnn_batchnorm_bidir_lstm.build_model(row['param_layers'], row['param_cnn_layers'], row['param_kernel_size'])
            else:
